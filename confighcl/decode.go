...
func decodeBodyToValue(body hcl.Body, ctx *hcl.EvalContext, val reflect.Value) hcl.Diagnostics {
	...
	case reflect.Struct:
		...
		_, diag := decodeBodyToStruct(body, ctx, val, schema, partial)
		return diag
	case reflect.Map:
...
func decodeBodyToStruct(body hcl.Body, ctx *hcl.EvalContext, val reflect.Value, schema *hcl.BodySchema, partial bool) (hcl.Body, hcl.Diagnostics) {
